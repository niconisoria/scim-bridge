...
async def remove_members(
    ...
    async def step_forward() -> None:
        for target_user_id in resolved:
            await client.remove_user_from_group(group_target_id, target_user_id)
            ctx["removed"].append(target_user_id)
        await store.cache_del("group", str(group_target_id), "members")

    async def step_rollback() -> None:
        for uid in reversed(ctx["removed"]):
            try:
                await client.add_user_to_group(group_target_id, uid)
            except Exception:
                pass
        await store.cache_del("group", str(group_target_id), "members")

    await run_saga([Step("remove-members", step_forward, step_rollback)])
