...
@router.get("")
async def list_groups(params: Params, store: Store, client: Client) -> JSONResponse:
    if params.filter:
        resources = await _apply_filter(params.filter, store, client)
        total = len(resources)
    else:
        page = await client.list_groups(offset=params.startIndex - 1, page_size=params.count)
        resources = []
        for bg in page.data:
            if r := await _group_to_scim_response(bg, store):
                resources.append(r)
        total = page.count
...
async def _group_to_scim_response(bg, store: RedisStore) -> ScimGroupResponse | None:
    ...
    return brivo_group_to_scim(bg, scim_id, members, created_at, f"/scim/v2/Groups/{scim_id}")
...
async def _apply_filter(
    ...
    results = []
    for bg in await paginate_all(client.list_groups):
