...
Scope: `BrivoClient` class wrapping all user and group endpoints (list, get, create, update, delete, member add/remove). No saga logic, no SCIM mapping — pure Brivo API surface. Rate limiter (#20) plugs in via optional constructor arg.
...
- Optional `limiter` arg (any object with `async __aenter__/__aexit__`) — noop when `None`; #20 passes real limiter
...
AC:
1. `BrivoClient(http: AsyncClient, limiter=None)` — constructor; `limiter` used as async context manager before each call if not `None`
...
flowchart LR
    A([caller]) --> B{limiter?}
    B -- yes --> C[async with limiter]
    B -- no --> D[send HTTP request]
    C --> D
    D --> E{status}
    E -- 2xx --> F[parse model]
    F --> G([return])
    E -- 404 --> H([raise BrivoNotFoundError])
    E -- 429 --> I([raise BrivoRateLimitError])
    E -- other --> J([raise BrivoError])
```
...
class BrivoClient:
    def __init__(self, http: AsyncClient, limiter=None): ...
    # internal
...
Note: `page_size` maps to Brivo query param `pageSize` (camelCase). `_call` handles limiter context + HTTP dispatch. `_check` maps status → exception.
...
`BrivoClient` wraps all 14 Brivo endpoints via injected `httpx.AsyncClient`. Private `_call` acquires optional `limiter` async context manager before each HTTP dispatch; `_check` maps 404→`BrivoNotFoundError`, 429→`BrivoRateLimitError`, other non-2xx→`BrivoError`. `page_size` parameter serialized as `pageSize` in query string. Tests use `respx.mock` context manager — first respx consumer in the project.
