...
As concrete saga (#25-32), want a reusable step-runner with retries and rollback, so each saga defines only its step list without duplicating retry/rollback logic.
...
3. Step failure → execute completed steps' rollbacks in reverse order; skip steps with `rollback=None`
...
flowchart LR
    ...
    D --> E[rollback completed in reverse]
...
`run_saga` iterates steps sequentially, calling each `forward` once. On failure, rolls back all completed steps in reverse — skips `rollback=None`, swallows rollback exceptions (logs error, continues). Raises `SagaError(saga_id, failed_step)` after rollback. No saga-level retry — transient HTTP errors (429) are handled by `brivo_retry` at the client boundary, which is the correct layer; saga-level retry would catch non-retryable errors (404, 400) and stack badly with client retries.
