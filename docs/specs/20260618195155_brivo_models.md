...
Scope: `BrivoUser`, `BrivoUserWrite`, `BrivoGroup`, `BrivoGroupWrite`, `BrivoGroupRef`, `BrivoPaginatedList[T]`, `BrivoEmail`, `BrivoPhoneNumber`, `BrivoError`. All shapes from brivo-mock.md.
...
9. `BrivoError` has `code: int`, `message: str`
10. All models use `model_config = ConfigDict(populate_by_name=True)` for camelCase alias handling
