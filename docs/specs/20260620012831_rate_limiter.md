...
5. `brivo_retry` importable from `app.brivo.client` (lives alongside `BrivoRateLimitError` to avoid circular imports); `AsyncLimiter` instantiated inline at call site — no wrapper needed
...
- `app/brivo/client.py` — `brivo_retry` decorator + `@brivo_retry` on `BrivoClient._call`; `AsyncLimiter` instantiated inline in `main.py`
...
[app/brivo/client.py](app/brivo/client.py) [tests/unit/test_rate_limiter.py](tests/unit/test_rate_limiter.py)
