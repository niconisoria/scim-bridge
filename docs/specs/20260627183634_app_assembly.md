...
7. `RequestLoggingMiddleware` registered on app — logs every request as `http.request` via structlog; on unhandled exception logs `http.error` and re-raises; uvicorn access logs suppressed via `--no-access-log` in `Dockerfile`
8. `BrivoError` exception handler returns 502 — upstream Brivo errors don't crash the ASGI app; status 502 signals gateway failure without leaking Brivo internals to the IdP
