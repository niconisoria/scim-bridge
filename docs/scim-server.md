# SCIM Server

FastAPI service exposing SCIM 2.0 endpoints. Translates SCIM operations to Brivo API calls via the service layer. Multi-step operations delegated to the saga orchestrator (→ [saga.md](saga.md)). ID resolution via Redis (→ [redis.md](redis.md)).

## Endpoints

All routes prefixed `/scim/v2/`. Bearer token required on all except discovery.

### Users

| Method | Path | Operation |
|---|---|---|
| `POST` | `/Users` | Create user |
| `GET` | `/Users/{id}` | Get user |
| `PUT` | `/Users/{id}` | Full replace |
| `PATCH` | `/Users/{id}` | Partial update |
| `DELETE` | `/Users/{id}` | Delete user |
| `GET` | `/Users` | List / filter |

### Groups

| Method | Path | Operation |
|---|---|---|
| `POST` | `/Groups` | Create group |
| `GET` | `/Groups/{id}` | Get group |
| `PUT` | `/Groups/{id}` | Full replace |
| `PATCH` | `/Groups/{id}` | Partial update |
| `DELETE` | `/Groups/{id}` | Delete group |
| `GET` | `/Groups` | List / filter |

### Discovery (unauthenticated)

`GET /scim/v2/ServiceProviderConfig`, `/scim/v2/Schemas`, `/scim/v2/ResourceTypes`

Auth middleware skips these paths by prefix match — no token required.

## Authentication

Bearer token on all non-discovery routes. Token from `SCIM_BEARER_TOKEN` env var (via `pydantic-settings`).
Failure → `401` with SCIM error body. Constant-time string comparison.

## Resource ID Generation

`scim_id` is a UUID v4 generated by the bridge on `POST /Users` or `POST /Groups`. It is stable for the resource lifetime and stored in Redis alongside `brivo_id`. Brivo and Okta never see each other's IDs directly — `scim_id` is the bridge's own namespace.

## SCIM Schemas

### User
```json
{
  "schemas": ["urn:ietf:params:scim:schemas:core:2.0:User"],
  "id": "<scim-uuid>",
  "externalId": "<okta-id>",
  "userName": "john@example.com",
  "name": { "givenName": "John", "familyName": "Doe" },
  "emails": [{ "value": "john@example.com", "primary": true }],
  "phoneNumbers": [{ "value": "+15550001234", "primary": true }],
  "active": true,
  "meta": { "resourceType": "User", "created": "...", "lastModified": "...", "version": "W/\"..\"", "location": "/scim/v2/Users/<scim-uuid>" }
}
```

### Group
```json
{
  "schemas": ["urn:ietf:params:scim:schemas:core:2.0:Group"],
  "id": "<scim-uuid>",
  "externalId": "<okta-group-id>",
  "displayName": "Engineering",
  "members": [{ "value": "<scim-user-id>", "display": "john@example.com" }],
  "meta": { "resourceType": "Group", "created": "...", "lastModified": "...", "version": "W/\"..\"", "location": "/scim/v2/Groups/<scim-uuid>" }
}
```

### ListResponse
```json
{
  "schemas": ["urn:ietf:params:scim:api:messages:2.0:ListResponse"],
  "totalResults": 42,
  "startIndex": 1,
  "itemsPerPage": 10,
  "Resources": [...]
}
```

### PATCH Operations
```json
{
  "schemas": ["urn:ietf:params:scim:api:messages:2.0:PatchOp"],
  "Operations": [
    { "op": "add",     "path": "members",                            "value": [{ "value": "<scim-user-id>" }] },
    { "op": "remove",  "path": "members[value eq \"<scim-user-id>\"]"                                         },
    { "op": "replace", "path": "active",                             "value": false                           }
  ]
}
```

Supported `op` values: `add`, `remove`, `replace`.

#### PATCH Path Filter Parsing

`members[value eq "<scim-user-id>"]` is a filter-path expression. Parse with regex:
```
^(\w+)\[value eq "([^"]+)"\]$
→ attribute = members, value = <scim-user-id>
```
Unrecognized path format → `400` with `scimType: "invalidPath"`.

### Error
```json
{
  "schemas": ["urn:ietf:params:scim:api:messages:2.0:Error"],
  "status": 404,
  "detail": "User not found"
}
```

`scimType` present only for specific error conditions (e.g., `"noTarget"` on PATCH when path matches no attribute, `"uniqueness"` on 409).

### HTTP Status Codes

| Status | When |
|---|---|
| `200` | GET, PUT, PATCH success (with response body) |
| `201` | POST (create) success |
| `204` | DELETE success; PATCH success (no response body) |
| `400` | Invalid request body, unrecognized PATCH path, `displayName` > 35 chars |
| `401` | Missing / invalid token |
| `404` | Resource not found |
| `409` | Conflict — `externalId` already provisioned (`scimType: "uniqueness"`) |
| `429` | Rate limited — only propagated after all tenacity retries exhausted |
| `500` | Internal / saga failure |

## SCIM → Brivo Field Mapping (Write Path)

### User

| SCIM | Brivo |
|---|---|
| `externalId` | `externalId` |
| `name.givenName` | `firstName` |
| `name.familyName` | `lastName` |
| `emails[primary].value` | `emails[0].address` |
| `phoneNumbers[primary].value` | `phoneNumbers[0].number` |
| `active` | `suspended` (inverted: `active=true` → `suspended=false`) |

`userName` has no direct Brivo field — use `emails[0].address` as canonical identity.

### Group

| SCIM | Brivo |
|---|---|
| `displayName` | `name` (max 35 chars — enforce at validation layer, return `400` if exceeded) |
| `members[].value` (resolved to Brivo ID) | `PUT /groups/{groupId}/users/{userId}` per member |

### PUT Full-Replace Semantics

`PUT /Users/{id}` replaces all writable fields. Fields absent from the payload are cleared in Brivo where the field is nullable (e.g., `phoneNumbers`). Non-nullable fields (`firstName`, `lastName`, `emails`) must be present — absence → `400`.

## Brivo → SCIM Field Mapping (Read Path)

Applied on every GET, list, PUT/PATCH response.

### User

| Brivo | SCIM |
|---|---|
| resolved via Redis `scim:user` key | `id` (`scim_id`) |
| `externalId` | `externalId` |
| `firstName` | `name.givenName` |
| `lastName` | `name.familyName` |
| `emails[0].address` | `emails[0].value` (`primary: true`); also `userName` |
| `phoneNumbers[0].number` | `phoneNumbers[0].value` (`primary: true`) |
| `suspended` | `active` (inverted: `suspended=false` → `active=true`) |
| `created` | `meta.created` |
| `updated` | `meta.lastModified` |

### Group

| Brivo | SCIM |
|---|---|
| resolved via Redis `scim:group` key | `id` (`scim_id`) |
| `externalId` | `externalId` |
| `name` | `displayName` |
| members resolved via `ext:user:{externalId}` | `members[].value` (`scim_id` per member) |

#### Member Hydration

Brivo `GET /groups` and `GET /groups/{groupId}` do not return member lists. To populate `members[]`:

1. Call `GET /v1/api/groups/{groupId}/users` to fetch Brivo user list for the group
2. For each returned user, resolve `externalId` → `scim_id` via `ext:user:{externalId}` in Redis
3. If a user has no Redis mapping (provisioned outside the bridge), omit from `members[]` — do not fail

For `GET /Groups` (list): fetch members for each group in the page. This is N+1 Brivo calls per page — acceptable for this project's scope. Each call goes through the rate limiter.

### Partial Brivo Responses

Brivo may omit optional fields (e.g., `phoneNumbers`). Rule: **omit the field from the SCIM response** rather than returning `null` or an empty array. Never fail on a missing optional field.

## Meta Computation

| Field | Source |
|---|---|
| `meta.resourceType` | Hardcoded: `"User"` or `"Group"` |
| `meta.location` | `{SCIM_BASE_URL}/scim/v2/Users/{scim_id}` — base URL from `SCIM_BASE_URL` env var |
| `meta.version` | `W/"{sha1(stable_json_of_brivo_resource)}"` — deterministic hash of Brivo response |
| `meta.created` | Brivo `created` field |
| `meta.lastModified` | Brivo `updated` field |

## Pagination & Filtering

### Pagination

SCIM and Brivo use different pagination schemes — bridge translates:

| SCIM param | Default | Brivo param | Formula |
|---|---|---|---|
| `startIndex` | `1` | `offset` | `offset = startIndex - 1` |
| `count` | `100` | `pageSize` | `pageSize = count` |

`totalResults` comes from Brivo's `count` field. `itemsPerPage` = number of resources actually returned.

### Filtering

SCIM filter `filter=userName eq "x"` — Brivo list endpoints have no server-side filter support. Bridge fetches all pages from Brivo and filters in memory:

- `userName eq` → match against `emails[0].address`
- `displayName eq` → match against Brivo group `name`

For large datasets this is inefficient — acceptable for this project's scope.

Example: `GET /scim/v2/Users?filter=userName eq "john@example.com"&startIndex=1&count=10`

## References

| Resource | URL |
|---|---|
| RFC 7643 — SCIM Core Schema (Users, Groups, schemas) | https://www.rfc-editor.org/rfc/rfc7643 |
| RFC 7644 — SCIM Protocol (HTTP operations, filtering, pagination) | https://www.rfc-editor.org/rfc/rfc7644 |

## Saga Delegation

| Operation | Saga? |
|---|---|
| Create user | Yes → [saga.md § Create User](saga.md) |
| Delete user | Yes → [saga.md § Delete User](saga.md) |
| Create group | Yes → [saga.md § Create Group](saga.md) |
| PATCH add member | Yes → [saga.md § Add Member](saga.md) |
| PATCH remove member | Yes → [saga.md § Remove Member](saga.md) |
| PUT / PATCH replace | No — single Brivo call, tenacity retries |
